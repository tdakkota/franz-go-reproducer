...
A minimal reproducer for high memory usage in [franz-go](https://github.com/twmb/franz-go) when a consumer catches up after accumulating lag.

## What it does

- **producer** — publishes 5 MiB records to `test-topic` every 500 ms using zstd compression.
- **consumer** — consumes records with `franz-go`, POSTs each record payload to the sink over HTTP, then commits.
...
| consumer | `-concurrency`             | 2        | Max concurrent HTTP pushes / PollRecords max |
| consumer | `-fetch-max-bytes`         | 50 MiB   | `FetchMaxBytes` passed to franz-go   |
| consumer | `-fetch-max-partition-bytes` | 10 MiB | `FetchMaxPartitionBytes`             |
